...
<#macro showErrorsByKey separator code classOrStyle="">
    <#if code?exists>
	    <#if classOrStyle == "">
	        <b>${springMacroRequestContext.getMessage(code)}</b>
	    <#else>
	        <#if classOrStyle?index_of(":") == -1><#assign attr="class"><#else><#assign attr="style"></#if>
	        <span ${attr}="${classOrStyle}">${springMacroRequestContext.getMessage(code)}</span>
	    </#if>
    </#if>
</#macro>
<#function showErrorsPanel classOrStyle="">
            <#assign classOrStyle1="error">
            <#if classOrStyle?index_of(":") == -1><#assign attr="class"><#else><#assign attr="style"></#if>
	        <#local c = "<span  ${attr}='${classOrStyle1}'></span>">
	       <#return c>
</#function>
